...
- **RESOLVED (2026-07-20 15:49 UTC):** user applied the URL fix (commit `47c13c8` "Update default URL in alerts-cron workflow") and manually dispatched the workflow. Run `29756893518` (workflow_dispatch): **conclusion=success**, step "Ping the cron endpoint" → success. Last failure was 14:53 UTC (pre-fix). Verified zero `m4tiziddd` occurrences left on main; exact-step simulation returns HTTP 200 `{"ok":true,...}`. Background push pipeline is operational again. Note: GitHub throttles `*/5` scheduled runs on this repo to roughly every 2–3 h in practice — alerts are near-real-time only up to that granularity.
